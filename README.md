...
| 成本估算 | 总税负 = 基础税率（第二栏国家按第二栏）+ 301 + FLIP 301（按原产地与配置开关）；文本常驻「不含 AD/CVD、232 等未建模措施」，命中原产地类未建模标目时再标「另有 N 组待核」 | Web「成本估算」/ `--estimate` |
...
| AI 助手 | 商品归类 / 清单批量分析：召回走同一套三通道（语义与先例通道用原始中文描述），精排候选带归类路径与判定条件（批量与单条同源，12 条）；调用走服务端 JSON 模式、temperature 0 + 固定 seed（可复现）、提示词哈希落盘缓存（`.cache/llm/`，重跑同一清单零等待） | Web「清单归类」/「搜索与归类」 |
| 数据变动提醒 | 官方数据更新重建后自动 diff；Web 进程按数据库文件 mtime 自动换新库，重建后无需重启 | Web「数据变动」 |
| **232 类产品探测** | `scripts/extract_c99_products.py` 从 Chapter 99 PDF 提取按产品触发的 note（16 钢铝铜 / 33 乘用车 / 37 软木 / 38 中重型车 / 39 半导体 / 51 加拿大特定产品）列举的子目清单 → `data/c99_product_scopes.json`，构建时编译进库。编码落在清单里即标「落在 232 类产品清单，该税未计」，且 FLIP 301 按 note 52(f) 对此类产品不适用，FLIP 列标「232 存疑」（税额仍按不豁免计，少收比多收危险）。注意：note 正文不出现 "section 232" 字样，"232" 是按措辞与公告号的推断；清单只是必要条件，note 内另有含量 / 用途 / 技术参数条件 | 查询结果「备注」/「FLIP301」列 / 来源弹窗 |
| **评测集** | `scripts/eval_classify.py --build 300` 从 CROSS 镜像抽 2023 年后单编码 NY 裁定做金标（`data/eval/rulings_golden.json`，留一法剔除裁定号本身）；`eval_classify.py` 报各召回通道与融合的 recall@5 / @20，`--llm` 再报归类 top-1 / top-3。改检索、改提示词、换模型先跑它 | 命令行 |
| **未建模措施探测** | `htsdata.csv` 有 636 个 9903 标目，本工具只建模中国 301（84 个）与 FLIP 301（约 100 个）。其余 451 个（note 2 墨加等原产地标目、note 50 巴西、9903.02/.03 转运与全球档、232 类产品标目、配额与保障措施）**不在总税负里**。构建时按组记录每个标目提及的原产地；查询时凡标目正文提及该原产地（或以任何国家为条件的 note 2 标目），备注标「⚠ 另有未建模 9903 标目以该原产地为条件，总税负不完整」并在来源弹窗列出示例标目与税率。**只探测不判定**：这些标目的法律状态与是否适用由人核实 | 查询结果「备注」/ 来源弹窗 |
